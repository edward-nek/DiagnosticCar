...
public class CorrectLog extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_correct_log);

        ActionBar actionBar = getSupportActionBar();
        actionBar.setTitle("Редактирование данных: ");
        actionBar.setBackgroundDrawable(new ColorDrawable(Color.parseColor("#DB001B")));
        actionBar.setDisplayHomeAsUpEnabled(true);

        tvHelp = (TextView) findViewById(R.id.tvHelpCorrect);

        etDate = (EditText) findViewById(R.id.etDateCorrect);
        etTime = (EditText) findViewById(R.id.etTimeCorrect);

        etKolesa = (EditText) findViewById(R.id.etKolesaCorrect);
        ...
        etMaslo = (EditText) findViewById(R.id.etMasloCorrect);
        ...
        etAkkum = (EditText) findViewById(R.id.etAkkumCorrect);
        ...
        etOboroty = (EditText) findViewById(R.id.etOborotyCorrect);
        ...
        etBenzin = (EditText) findViewById(R.id.etBenzinCorrect);
        ...
        etTemper = (EditText) findViewById(R.id.etTemperCorrect);
        ...
        swRul = (SwitchCompat) findViewById(R.id.swRulCorrect);
        swCar = (SwitchCompat) findViewById(R.id.swCarCorrect);
        ...
        cursor = dataDB.getRec(MainActivity.idCorrect);
        if (cursor.moveToFirst()){
            ...
            boolean check = true;
            int i = -1;
            while (check && (i < dateTime.length())){
                i++;
                if (dateTime.charAt(i) == ' '){
                    check = false;
                }
            }
            etDate.setText(dateTime.substring(0, i));
            etTime.setText(dateTime.substring(i+1, dateTime.length()));

            etOboroty.setText(cursor.getString(oborotyDvsIndex));
            etKolesa.setText(cursor.getString(pressureWheelsIndex));
            etAkkum.setText(cursor.getString(voltageIndex));
            etTemper.setText(cursor.getString(temperatureIndex));
            etBenzin.setText(cursor.getString(gasConsumptionIndex));
            etMaslo.setText(cursor.getString(pressureOilIndex));
            if (cursor.getInt(bienieRulyaIndex) == 1){
                swRul.setChecked(true);
            }
            else{
                swRul.setChecked(false);
            }
            if (cursor.getInt(carShocksIndex) == 1){
                swCar.setChecked(true);
            }
            else{
                swCar.setChecked(false);
            }
        }

        btSave = (Button) findViewById(R.id.btSaveCorrect);
            ...
            public void onClick(View v) {
                ...
                if ((etDate.getText().toString().length() > 0)&&(etTime.getText().toString().length() > 0)&&
                    ...
                    if (checkInput()){
                        ...
                        Intent intent = new Intent(CorrectLog.this, MainActivity.class);
                    ...
                    Toast.makeText(CorrectLog.this,"Для начала заполните поля!", Toast.LENGTH_LONG).show();
                }
            }
        });
    }
    ...
    public void saveLog(){
        ...
        dataDB.delRec(MainActivity.idCorrect);
        dataDB.addRec(MainActivity.idCorrect, date, oboroty_dvs, pressure_wheels, voltage, temperature, gas_consumption,
    ...
    public void setDate() {
        new DatePickerDialog(CorrectLog.this, d,
    ...
    public void setTime() {
        new TimePickerDialog(CorrectLog.this, t,
    ...
    public boolean checkInput(){
        ...
        if (!check){
            Toast.makeText(CorrectLog.this, errorLog, Toast.LENGTH_LONG).show();
        }
        return check;
    }
